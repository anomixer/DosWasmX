...
class MyClass {
    ...
    configureEmulator() {
        ...
        // Start polling MT-32 LCD Messages
        if (this.lcdInterval) clearInterval(this.lcdInterval);
        this.lcdInterval = setInterval(() => {
            if (this.getMt32LcdMessage) {
                try {
                    let message = this.getMt32LcdMessage();
                    let lcdTextEl = document.getElementById('mt32-lcd-text');
                    let lcdEl = document.getElementById('mt32-lcd');
                    if (message && message.trim() !== '') {
                        if (lcdTextEl && lcdTextEl.innerText !== message) {
                            lcdTextEl.innerText = message;
                            if (lcdEl) lcdEl.style.display = 'inline-block';
                        }
                    } else {
                        if (lcdEl && lcdEl.style.display !== 'none') {
                            lcdEl.style.display = 'none';
                        }
                    }
                } catch (e) {
                    console.error('Error fetching MT-32 LCD message:', e);
                }
            }
        }, 200);
    ...
    async LoadEmulator(byteArray) {
        ...
        this.sendDosControls = Module.cwrap('neil_send_dos_controls', null,
            ['string', 'string', 'string', 'array', 'number']); //arrays are always unsigned byte arrays
        this.getMt32LcdMessage = Module.cwrap('neil_get_mt32_lcd_message', 'string', []);
    ...
    async initAudio() {
        ...
        if (!this.audioInited) {
            ...
            // Resume audio context on user interaction to bypass autoplay restrictions (e.g. on Firefox)
            const resumeAudio = () => {
...
window.onerror = function (message, source, lineno, colno, error) {
    ...
    if (error && error.stack) {
        ...
        myClass.onError(message + '<br><pre>' + error.stack + '</pre>');
    } else {
...
window.onunhandledrejection = function (error) {
    ...
    if (reason && reason.stack) {
        ...
        myClass.onError('Unhandled Promise Rejection: ' + msg + '<br><pre>' + reason.stack + '</pre>');
    } else {
        myClass.onError('Unhandled Promise Rejection: ' + msg);
    }
}
