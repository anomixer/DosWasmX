...
extern void set_autoexec_internal();
...
isoDrive::isoDrive(char driveLetter, const char* fileName, uint8_t mediaid, int& error, std::vector<std::string>& options) {
	...
	sprintf(iso_drive_mount, "imgmount d \"%s\"\r\n", fileName);
	set_autoexec_internal();
	printf("iso drive mounted %s\n", fileName);
	...
	if (IS_DIR(FLAGS2)) {
		if (de->fileIdentLength == 1 && de->ident[0] == 0) strcpy((char*)de->ident, ".");
			...
			if (is_joliet) {
				...
				CodePageHostToGuestUTF16((char*)de->ident,(const uint16_t*)de->ident);
			}
		}
	} else {
		if (de->fileIdentLength > 200) return -1;
